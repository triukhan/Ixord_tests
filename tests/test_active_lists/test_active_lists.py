...
def test_add_file(auto_reg, rand):
    ...
    af = requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2],
                       json=[{"stepId": sid, "dataPatch": [{"op": "add", "path": "/value/blocks/0", "value":
                           {"type": "attaches", "data": {"file": {"url": url}}}}]}], headers=auto_reg[0], )
    Response(af).assert_status_code(200).validate(Post)


def test_complete_by_done(auto_reg):
    sid = requests.get(ENDPOINT_SID + auto_reg[3], headers=auto_reg[0]).json()["result"]["steps"][0]["id"]
    cl = requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2],
                       headers=auto_reg[0], json=[{"stepId": sid, "executed": 100, "dataPatch": [], "completed": True}])
...
def test_drd__list_elements(auto_reg, rand):
    ...
    requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2], headers=auto_reg[0],
                  json=[{"stepId": sid, "dataPatch": [{"op": "add", "path": "/value/blocks/0", "value":
    ...
    drd = requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2],
...
def test_delete_element(auto_reg, rand):
    ...
    requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2], headers=auto_reg[0],
                  json=[{"stepId": sid, "dataPatch": [{"op": "add", "path": "/value/blocks/0", "value":
                             {"type": "paragraph", "data": {"text": "text"}}}]}])
    de = requests.post(ENDPOINT + "defaultExecuteExecution/executeSteps?executionId=" + auto_reg[2], headers=auto_reg[0],
                       json=[{"stepId": sid, "dataPatch": [{"op": "remove", "path": "/value/blocks/0"}]}])
